...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < result.length + 2; i += 1) {
    while (result.includes(brackets1)) {
      result = result.replace(brackets1, '');
    }
    while (result.includes(brackets2)) {
      result = result.replace(brackets2, '');
    }
    while (result.includes(brackets3)) {
      result = result.replace(brackets3, '');
    }
    while (result.includes(brackets4)) {
      result = result.replace(brackets4, '');
    }
  }
...
function evaluateTicTacToePosition(position) {
  ...
  console.log(position[2][0] === position[2][1] && position[2][0] === position[2][2]);
  for (let i = 0; i < 3; i += 1) {
